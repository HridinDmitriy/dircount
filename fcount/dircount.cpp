...
void directory_traversal(const string& _dir, size_t& _counter, bool _empty_too, int _dispay_count, string _mask);
...
int main(int argc, const char* argv[]) 
{
	...
	try
	{
		...
		{
			directory_traversal<rdir_it>(work_directory, directories_count, empty, display_count, mask);
		}
		else 
		{
			directory_traversal<dir_it>(work_directory, directories_count, empty, display_count, mask);
		}
...
void directory_traversal(const string & _dir, size_t & _counter, bool _empty_too, int _dispay_count, string _mask)
{
	...
	{ 
		...
		if (is_directory && (_empty_too || (!_empty_too && !is_dir_empty))
			&& (!has_mask || (has_mask && matching)))
		{
			++_counter;
			if (_dispay_count-- > 0)
			{ 
